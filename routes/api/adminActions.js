...
router.post('/addLocation',[
    check('locationName', 'Please include a location name with 4 or more characters').isLength({min: 4}),
    ...
    try{
        ...
        if(location){
            return res.status(400).json({errors:[{msg: 'Location with these coordinates exists'}]});
        }
        
        location = await CycleonLocationModel.findOne({locationName});
        if(location){
            return res.status(400).json({errors:[{msg: 'Location with this name exists'}]});
        }
...
router.post('/addBike',[
    check('name', 'Please enter a bike name with 4 or more characters').isLength({min: 2}),
    check('description', 'Please enter a description with 10 or more characters').isLength({min: 10}),
    ...
    try{
        ...
        location = await CycleonLocationModel.findOneAndUpdate({_location: locationName}, { $push: { bikes: {name, description, _id: idGenerator.makeid(24)}} },  {new: true})
        res.send(location)
    } catch(err) {
        console.error(err.message);
        return res.status(500).send('Server Error')
...
router.delete('/deleteBike',[
    ...
    try{
        ...
        if(location.bikes.length == location_updated.bikes.length){
            return res.status(400).json({errors:[{msg: `Bike does not exist`}]});
        }
        res.send(location)
    } catch(err) {
        console.error(err.message);
...
router.delete('/deleteLocation', adminAuth, async (req,res)=>{
    const {locationName, bikeId} = req.body;
    try{
        let location = await CycleonLocationModel.findOneAndDelete({locationName});
        if(!location){
            return res.status(400).json({errors:[{msg: `Location with the name ${locationName} does not exist`}]});
        }
        res.send(location)
